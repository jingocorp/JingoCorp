...
const handleSubmit = async (e) => {
	...
	const res = await fetch("/admin" , {
		...
		body : JSON.stringify({
			modelName ,image ,type ,quantity ,rate ,state ,city ,substation
		}) 
	}) ;
	...
	if(!data  ||  res.status === 400  ||  res.status === 422) {
		window.alert("VehicleRegistration Unsuccessful :( \n Please fill the all the fields properly !!") ;
		console.log("VehicleRegistration Unsuccessful") ;
	}else {
		window.alert("VehicleRegistration submitted Successful from frontend :)") ;
		console.log("VehicleRegistration submitted Successful from frontend") ;
		// navigate("/login") ;
	}
	...
	<div className="signup">
		...
		>
            ...
            <Form.Item
				...
				hasFeedback
			>
				<Input type="text" value={substation} 
			...
			<div className="bt">
				<button
					...
					style={{ marginTop: "15px", marginLeft: "10px", display: 'flex',alignItems: 'center', justifyContent: 'center', fontSize: '20px', height: '2.2em', width: '12em'}}
				>
					Register
				</button>
			</div>
